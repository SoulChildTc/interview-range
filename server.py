...
def batch_grade(session, batch_size=10):
    ...
    def _grade_chunk(chunk, start_no):
        ...
        prompt = (
            ...
            "重要判定口径，严格遵循：\n"
            "1. 面试官没有追问到的知识点 = 「未考察」，绝不是遗漏或弱点，不扣分；\n"
            "2. 面试官追问后、候选人答出来了（即使初始回答未主动提及）= 已掌握，不是遗漏也不是弱点，"
            "绝不计入 weaknesses/missed_points，不因此扣分；\n"
            "3. 只有面试官明确追问了、候选人仍答错/答不出/含糊反复避开 = 才算遗漏或弱点，才扣分。\n"
